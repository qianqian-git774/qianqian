...
"""
resume-vibe 本地服务器
- 静态文件服务（等同于 python -m http.server）
- POST /api/match  →  调用阿里云百炼 qwen-plus
"""
...
# macOS Python 常见 SSL 证书问题修复
ssl_ctx = ssl.create_default_context()
try:
    import certifi
    ssl_ctx = ssl.create_default_context(cafile=certifi.where())
except ImportError:
    # 没有 certifi 时跳过证书验证（仅本地开发使用）
...
def call_qwen(jd: str) -> dict:
    ...
    if not api_key:
        raise ValueError("QWEN_API_KEY 未配置，请检查 .env.local")
